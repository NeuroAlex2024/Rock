...
const saveNickBtn = byId("saveNickBtn");
...
const copyLinkBtn2 = byId("copyLinkBtn2");
...
async function init(){
  const saved = localStorage.getItem("rpsls_nick");
  nickInput.value = saved || "";
  myName = shortName(saved || "Гость123");
  saveNickBtn.addEventListener("click", () => {
    const v = shortName(nickInput.value || "Гость123");
  ...
  copyLinkBtn.addEventListener("click", () => copy(location.href));
  copyLinkBtn2.addEventListener("click", () => copy(location.href));
...
function revealOutcome(round, code, roundId){
  ...
  resultBanner.classList.add("hidden");
  revealLeft.className = "emoji left slide-in-left clash";
  revealRight.className = "emoji right slide-in-right clash";
  revealLeft.textContent = aMove ? MOVE_EMOJI[aMove] : "❌";
  revealRight.textContent = bMove ? MOVE_EMOJI[bMove] : "❌";
  reasonText.textContent = REASONS_RU[reasonKey] || "";
